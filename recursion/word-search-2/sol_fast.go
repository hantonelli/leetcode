		...
		for i := 0; i < len(board) && !wordFound; i++ {
			for j := 0; j < len(board[0]); j++ {
				if words[k][0] == board[i][j] {
					if process2(i, j, &board, &visited, words[k], 1) {
...
func process2(i, j int, board *[][]byte, visited *map[[2]int]bool, word string, pos int) bool {
	if pos == len(word) {
		return true
	}
	...
	if 0 <= i-1 && !(*visited)[[2]int{i - 1, j}] && word[pos] == (*board)[i-1][j] {
		...
		if process2(i-1, j, board, visited, word, pos+1) {
			return true
		}
	...
	if 0 <= j-1 && !(*visited)[[2]int{i, j - 1}] && word[pos] == (*board)[i][j-1] {
		...
		if process2(i, j-1, board, visited, word, pos+1) {
			return true
		}
